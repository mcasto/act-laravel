# ACT Website — What's Working, What's Not (as of 2026-08-24)
...
- **Fixr ticket sales** — real purchases through Fixr now get automatically recorded (wired up this session).
...
- **Site settings** — default ticket price, sold-out threshold, contact emails, plus the content editors for the Support Us and Flex pages, all in one settings screen.
- **Payment instruction templates** ("Payment Methods" tab inside Site Settings — worth knowing this is a *different* thing from the payment-methods list below, despite the shared name) — editable text for how each payment option is explained to a buyer.
- **Site-wide announcement banner** — toggle on/off, rich text.
- **Angels donor program** — donation levels, individual donor records, and a benefits list per level.
...
- **Buying a flex ticket package**: the purchase page and its settings work, but there's no actual "process this purchase" step behind 
...
- **The underlying payment-methods list** (PayPal, Credit Card, Flex, etc., including their colors) works fine behind the scenes, but there's no screen to edit it — changes require going directly into the database.
...
- **"Message Us"** (a second, simpler contact widget elsewhere on the site): only emails you, nothing is saved — different behavior than the similarly-named Contact form, worth knowing if that's not what you'd expect.
...
If any of the "partly built" items are worth closing the gap on, the volunteer-admin one is the easiest win — it sounds like it just needs its menu link turned back on.
